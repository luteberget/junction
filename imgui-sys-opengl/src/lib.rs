...
impl Renderer {
    ...
    F: FnMut(&'static str) -> *const ::std::os::raw::c_void
  {
    ...
    unsafe {
      ...
      gl.TexImage2D(gl::TEXTURE_2D, 0, gl::RGBA as _, width as _, height as _, 
                    0, gl::RGBA, gl::UNSIGNED_BYTE, pixels as _ );
  ...
  pub fn render<'ui>(
    ...
    unsafe {
      ...
      let (width, height) = 
          ((*io).DisplaySize.x,
           (*io).DisplaySize.y);
          //(0,0);//ui.imgui().display_size();

      let fb_width = width * (*io).DisplayFramebufferScale.x;
      let fb_height = height * (*io).DisplayFramebufferScale.y;
      ...
      gl.VertexAttribPointer(
          self.locs.position, 2, gl::FLOAT,         gl::FALSE, 
          mem::size_of::<ImDrawVert>() as _, field_offset::<ImDrawVert, _, _>(|v| &v.pos) as _);
      gl.VertexAttribPointer(
          self.locs.uv,       2, gl::FLOAT,         gl::FALSE, 
          mem::size_of::<ImDrawVert>() as _, field_offset::<ImDrawVert, _, _>(|v| &v.uv) as _);
      gl.VertexAttribPointer(
          self.locs.color,    4, gl::UNSIGNED_BYTE, gl::TRUE,  
          mem::size_of::<ImDrawVert>() as _, field_offset::<ImDrawVert, _, _>(|v| &v.col) as _);


      igRender();
      let p_draw_data = igGetDrawData();

      //draw_data.scale_clip_rects(ui.imgui().display_framebuffer_scale());
      ImDrawData_ScaleClipRects(p_draw_data, (*io).DisplayFramebufferScale);

      //for draw_list in &draw_data {
      for i in 0..((*p_draw_data).CmdListsCount) {
        let draw_list = *(((*p_draw_data).CmdLists).offset(i as isize));
        gl.BindBuffer(gl::ARRAY_BUFFER, self.vbo);
        gl.BufferData(gl::ARRAY_BUFFER, 
                      ((*draw_list).VtxBuffer.Size as usize * mem::size_of::<ImDrawVert>()) as _, 
                       (*draw_list).VtxBuffer.Data as _, gl::STREAM_DRAW);

        gl.BindBuffer(gl::ELEMENT_ARRAY_BUFFER, self.ebo);
        gl.BufferData(gl::ELEMENT_ARRAY_BUFFER, 
                      ((*draw_list).IdxBuffer.Size as usize * mem::size_of::<ImDrawIdx>()) as _, 
                       (*draw_list).IdxBuffer.Data as _, gl::STREAM_DRAW);

        let mut idx_start = 0;
        for cmd_i in 0..((*draw_list).CmdBuffer.Size) {
        //for cmd in draw_list.cmd_buffer {
          let cmd = *((*draw_list).CmdBuffer.Data.offset(cmd_i as isize));
          if let Some(_callback) = cmd.UserCallback {
            unimplemented!("Haven't implemented user callbacks yet");
          } else {
            gl.BindTexture(gl::TEXTURE_2D, cmd.TextureId as _);
            gl.Scissor(cmd.ClipRect.x as GLint,
                       (fb_height - cmd.ClipRect.w) as GLint,
                       (cmd.ClipRect.z - cmd.ClipRect.x) as GLint,
                       (cmd.ClipRect.w - cmd.ClipRect.y) as GLint);
            gl.DrawElements(
                gl::TRIANGLES, cmd.ElemCount as _, 
                if mem::size_of::<ImDrawIdx>() == 2 { gl::UNSIGNED_SHORT } else { gl::UNSIGNED_INT }, 
                idx_start as _);
          }
          idx_start += cmd.ElemCount * mem::size_of::<ImDrawIdx>() as u32;
        }
      }
      ...
      //Use this to restore state if you are rendering other stuff
